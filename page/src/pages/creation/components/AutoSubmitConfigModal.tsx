...
const PROMPT_COUNT_OPTIONS = [1, 2, 3, 4];
const IMAGES_PER_PROMPT_OPTIONS = [1, 2, 3, 4];

interface AutoSubmitConfigModalProps {
  visible: boolean;
  initialConfig: AutoSubmitConfig;
  onConfirm: (config: AutoSubmitConfig) => void;
  onCancel: () => void;
}

export default function AutoSubmitConfigModal({
  visible,
  initialConfig,
  onConfirm,
  onCancel,
}: AutoSubmitConfigModalProps) {
  const [config, setConfig] = useState<AutoSubmitConfig>(initialConfig);
        ...
        <div className="flex flex-col gap-4">
          <div>
            <label
              ...
              style={{ fontFamily: "'ZCOOL KuaiLe', cursive" }}
            >
              <i className="ri-stack-line mr-1"></i>生成的Prompt数量
            </label>
            <div
            ...
            <p className="text-xs text-rose-300/50 mt-1.5">若生成数量少于设定值，则全部提交</p>
          </div>
          ...
          <div>
            <label
              ...
              style={{ fontFamily: "'ZCOOL KuaiLe', cursive" }}
            >
              <i className="ri-image-line mr-1"></i>每个 Prompt 生成几张图片
            </label>
            <div
              className="flex items-center gap-1 p-1 rounded-2xl"
              style={{

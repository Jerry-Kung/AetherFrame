...
class TestFileUploadSchemas:
    ...
    def test_failed_upload_info(self):
        ...
        info = FailedUploadInfo(original_filename="invalid.txt", error="文件类型不支持")
    ...
    def test_reference_images_upload_response(self):
        ...
        failed = [
            FailedUploadInfo(original_filename="invalid.txt", error="文件类型不支持")
        ]

        response = ReferenceImagesUploadResponse(
            uploaded=uploaded, failed=failed, total=2, task_id="task_123"
        )
...
class TestFileServiceIntegration:
    ...
    def test_repair_file_service_ensure_task_dirs(self, temp_data_dir, test_task):
        ...
        main_dir, refs_dir, results_dir = repair_file_service.ensure_task_dirs(
            test_task.id
        )
    ...
    def test_repair_file_service_list_reference_images_empty(
        self, temp_data_dir, test_task
    ):
        """测试列出参考图 - 空目录"""
        from app.services.repair_service import repair_file_service
    ...
    def test_repair_file_service_list_result_images_empty(
        self, temp_data_dir, test_task
    ):
        """测试列出结果图 - 空目录"""
        from app.services.repair_service import repair_file_service
...
class TestWithRealImage:
    ...
    REAL_IMAGE_PATH = os.path.join(
        os.path.dirname(os.path.dirname(__file__)), "data", "picture.png"
    )

    @pytest.mark.skipif(
        not os.path.exists(REAL_IMAGE_PATH), reason="data/picture.png 不存在"
    )

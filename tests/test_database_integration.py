...
class TestDatabaseConnection:
    ...
    def test_get_db_yields_session(self, temp_data_dir):
        ...
        task = RepairTask(id="test_conn", name="测试连接", prompt="test", output_count=1)
        db.add(task)
        db.commit()
...
class TestPromptTemplateRepository:
    ...
    def test_list_builtin(self, db_session):
        ...
        repo.create({"label": "内置1", "text": "test", "is_builtin": True, "sort_order": 2})
        repo.create({"label": "内置2", "text": "test", "is_builtin": True, "sort_order": 1})
    ...
    def test_list_all(self, db_session):
        ...
        repo.create({"label": "内置1", "text": "test", "is_builtin": True, "sort_order": 1})
        repo.create({"label": "自定义1", "text": "test", "is_builtin": False})
    ...
    def test_update_template(self, db_session):
        ...
        updated = repo.update(template.id, {
            "label": "新标签",
            "text": "新内容"
        })
...
class TestBaseRepository:
    ...
    def test_base_repository_get_by_id(self, db_session):
        ...
        task = RepairTask(id="base_test", name="Base测试", prompt="test", output_count=1)
        db_session.add(task)
        db_session.commit()
    ...
    def test_base_repository_count(self, db_session):
        ...
        for i in range(3):
            task = RepairTask(id=f"count_{i}", name=f"任务{i}", prompt="test", output_count=1)
            db_session.add(task)
        db_session.commit()

...
> 用户的原始输入可能包含文学化、抒情化的描述（如"发丝如丝绸般流淌"、"展现极致的信任感"）。这些内容是你理解场景与情绪的**创意来源**，但**禁止原样抄入模板字段**。填写模板时必须把它们转译为可直接绘制的视觉事实：具体的物体、位置、姿势、颜色、材质（"发丝如丝绸般流淌在榻榻米上" → "长发散开铺在榻榻米上"）；情绪意图转译为白名单内的单一表情与视线方向（"展现极致的信任感" → "柔和浅笑，看向镜头"）。

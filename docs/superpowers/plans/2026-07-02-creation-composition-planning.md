...
### Task S3-1: step1 任务步骤 0 增机位方位与高度 + 反 fetish 互斥 + 结构化持久化
...
- Modify: `app/services/creation_service/prompt_precreation_service.py` — `_build_step1_prompt` 无论 auto/manual 分支都渲染机位维度枚举;`_parse_step1_composition` 扩集到 `shooting_angle` / `camera_height`;写入 card 的 `composition` 字段;反 fetish 互斥规则以 Prompt 文案落地
...
- Test: `tests/services/test_prompt_precreation_composition_persistence.py::test_build_step1_prompt_contains_back_glance_mutex_rule`
...
def test_build_step1_prompt_contains_back_glance_mutex_rule():
    from app.services.creation_service.prompt_precreation_service import _build_step1_prompt
    p = _build_step1_prompt(chara_profile="cp", seed_prompt="sp")
    assert "回眸" in p and ("回头" in p or "上半身" in p) and "镜头" in p
...
Run: `pytest tests/services/test_prompt_precreation_composition_persistence.py -v -k "shooting or camera or back_glance or bias_placeholder"`
Expected: FAIL
...
_STEP0_CAMERA_TEMPLATE = (
    ...
    "{camera_height_enum}\n"
    "**反 fetish 互斥规则**:若你选择 `back_glance`(背面/回眸),必须在「角色姿势」段明确说明「角色上半身回头朝向镜头、视线接触观者」,以避免演变为纯臀腿后部构图。\n"
...
git commit -m "feat(S3): step1 引入机位方位+高度决策与反 fetish 互斥规则"
```
...
_STEP0_CAMERA_TEMPLATE = (
    ...
    "{gaze_direction_enum}\n"
    "**反 fetish 互斥规则**:若你选择 `back_glance`(背面/回眸),必须在「角色姿势」段明确说明「角色上半身回头朝向镜头、视线接触观者」;此时 gaze_direction 建议选 `to_camera`。\n"
...
5. **S3 验证**:同一 seed 跑 N 张,查看每张 card 的 `composition.shooting_angle` × `camera_height` 组合分布是否分散;抽取一张 `shooting_angle = back_glance` 的 Prompt,确认「角色姿势」段明确写了「上半身回头朝向镜头」。
